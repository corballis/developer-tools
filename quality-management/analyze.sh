...
SONAR_SERVER_URL=${SONAR_SERVER_URL:-http://localhost:9001}
...
TASK_URL=$(mvn -DskipTests sonar:sonar -Dsonar.projectKey=$PROJECT_KEY -Dsonar.projectName=$PROJECT_KEY | tee out | grep -Eo 'http.*/api/ce/task.*')
...
counter=0
while [ $(curl -s $TASK_URL 2>&1 | grep -q 'IN_PROGRESS'; echo $?) -eq 0 ]&&[ $counter -lt $CONNECT_RETRY ]; do
  sleep 5
  ((counter++))
...
ANALYSIS_ID=$(curl -s $TASK_URL | sed -e 's/[{}]/''/g' | awk -v RS=',"' -F: '/^analysisId/ {print $2}' | grep -oP '"\K[^"]+')
...
if [[ $(curl -s $ANALYSIS_URL 2>&1 | grep "ERROR" | wc -l) -gt 0 ]]; then
